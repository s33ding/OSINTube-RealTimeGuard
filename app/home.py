...
if run_button and input_data and not st.session_state.stop_requested:
    ...
    try:
        ...
        if df.empty:
            ...
            # Log to DynamoDB
            video_title = df['title'].iloc[0] if not df.empty and 'title' in df.columns else 'Unknown'
            comments_found = len(df)
            log_search_to_dynamodb(input_data, s3_key, comments_maxResult, search_limit, video_title, comments_found)
